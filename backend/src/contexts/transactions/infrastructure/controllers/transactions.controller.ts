...
export class TransactionsController {
  ...
  @ApiCreatedResponse({ type: TransactionResponseDto, description: 'Transaction processed (check status field for result)' })
  @ApiBadRequestResponse({ description: 'Invalid input, product not found, or out of stock' })
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Get transaction by ID', description: 'Returns the current state of a transaction.' })
  @ApiParam({ name: 'id', description: 'Transaction UUID', example: 'a1b2c3d4-e5f6-7890-abcd-ef1234567890' })
  @ApiOkResponse({ type: TransactionResponseDto, description: 'Transaction found' })
