  ...
  return (
    <svg className={className} viewBox="0 0 780 500" xmlns="http://www.w3.org/2000/svg">
      ...
      <text x="390" y="370" textAnchor="middle" fill="white" fontSize="290"
        fontFamily="Arial, sans-serif" fontWeight="bold" fontStyle="italic">VISA</text>
    </svg>
  );
}
...
const inputCls = 'w-full rounded-xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-900 placeholder-gray-400 outline-none transition focus:border-gray-900 focus:bg-white focus:ring-2 focus:ring-gray-900/10';

/* ─── card preview widget ─────────────────────────────────────── */

function CardPreview({
  number, holder, expMonth, expYear, cvc, brand, flipped,
}: {
  number: string; holder: string; expMonth: string; expYear: string;
  cvc: string; brand: CardBrand; flipped: boolean;
}) {
  const displayNumber = maskCardDisplay(number);
  const displayHolder = holder.trim() || 'FULL NAME';
  const displayExp = expMonth && expYear
      ...
      >
          ...
          <div>
            ...
            <div className="mt-3 flex items-end justify-between">
              <div>
                ...
                <p className="text-sm font-semibold text-white uppercase tracking-widest truncate max-w-[160px]">{displayHolder}</p>
              </div>
        ...
        >
          ...
          <div className="mt-4 mx-5 flex items-center justify-end rounded bg-white/10 px-3 py-2">
            <p className="font-mono text-white/80 text-sm tracking-widest">{cvc || '•••'}</p>
          </div>
...
export default function StepCardDelivery() {
  ...
  function validate(): boolean {
    ...
    if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(customer.email)) e.custEmail = 'Enter a valid email';
  ...
  const personalSection = (
    <div className="flex flex-col gap-3">
      ...
      <Field label="Phone" error={errors.custPhone}>
        <input type="tel" className={inputCls} placeholder="+57 300 000 0000"
    ...
    <div className="flex flex-col gap-3">
      <Field label="Card number" error={errors.cardNumber || errors.cardBrand}>
        <div className="relative">
          <input className={`${inputCls} pr-14`} placeholder="0000 0000 0000 0000"
            inputMode="numeric" value={card.number}
            onChange={(e) => setLocalCard((p) => ({ ...p, number: formatCardNumber(e.target.value) }))} />
      ...
      <Field label="Cardholder name" error={errors.cardHolder}>
        <input className={inputCls} placeholder="As it appears on card"
          value={card.holder}
          onChange={(e) => setLocalCard((p) => ({ ...p, holder: e.target.value.toUpperCase() }))} />
      </Field>
      <div className="grid grid-cols-3 gap-3">
        <Field label="Month" error={errors.expMonth}>
          <input className={inputCls} placeholder="MM" inputMode="numeric" maxLength={2}
            value={card.expMonth}
            onChange={(e) => setLocalCard((p) => ({ ...p, expMonth: e.target.value.replace(/\D/g, '').slice(0, 2) }))} />
        </Field>
        <Field label="Year" error={errors.expYear}>
          <input className={inputCls} placeholder="YY" inputMode="numeric" maxLength={2}
            value={card.expYear}
            onChange={(e) => setLocalCard((p) => ({ ...p, expYear: e.target.value.replace(/\D/g, '').slice(0, 2) }))} />
        </Field>
        <Field label="CVC" error={errors.cvc}>
          <input id={`${uid}-cvc`} className={inputCls} placeholder="•••"
            inputMode="numeric" maxLength={4} value={card.cvc}
            onFocus={() => setFlipped(true)} onBlur={() => setFlipped(false)}
            onChange={(e) => setLocalCard((p) => ({ ...p, cvc: e.target.value.replace(/\D/g, '').slice(0, 4) }))} />
        </Field>
      </div>
    </div>
  );
  ...
  const deliverySection = (
    <div className="flex flex-col gap-3">
      <Field label="Recipient name" error={errors.delName}>
        <input className={inputCls} placeholder="Who receives the package"
          value={delivery.recipientName}
          onChange={(e) => setLocalDelivery((p) => ({ ...p, recipientName: e.target.value }))} />
      </Field>
      <Field label="Contact phone" error={errors.delPhone}>
        <input type="tel" className={inputCls} placeholder="+57 310 000 0000"
          value={delivery.phone}
          onChange={(e) => setLocalDelivery((p) => ({ ...p, phone: e.target.value }))} />
      </Field>
      <Field label="Street address" error={errors.delAddress}>
        <input className={inputCls} placeholder="Calle 123 # 45-67"
          value={delivery.address}
          onChange={(e) => setLocalDelivery((p) => ({ ...p, address: e.target.value }))} />
      </Field>
      <div className="grid grid-cols-2 gap-3">
        <Field label="City" error={errors.delCity}>
          <input className={inputCls} placeholder="Bogotá"
            value={delivery.city}
            onChange={(e) => setLocalDelivery((p) => ({ ...p, city: e.target.value }))} />
        </Field>
        <Field label="Department" error={errors.delDept}>
          <input className={inputCls} placeholder="Cundinamarca"
            value={delivery.department}
            onChange={(e) => setLocalDelivery((p) => ({ ...p, department: e.target.value }))} />
        </Field>
      </div>
    </div>
  );
  ...
  const stepDots = (dark = false) => (
    <div className="flex items-center gap-1.5">
      {[1, 2, 3].map((n) => (
        <span key={n} className={`block rounded-full transition-all ${
          n === 1
    ...
    <>
          ...
          <div className="mt-6 mb-6 flex justify-center" style={{ perspective: '1000px' }}>
            <div style={{ width: '100%', maxWidth: '320px' }}>
              <CardPreview
                ...
                cvc={card.cvc} brand={brand} flipped={flipped}
              />
            </div>
          </div>

          <section className="mb-6">
            <h2 className="mb-3 text-xs font-bold text-gray-400 uppercase tracking-widest">Personal Info</h2>
            {personalSection}
          </section>
          <section className="mb-6">
            <h2 className="mb-3 text-xs font-bold text-gray-400 uppercase tracking-widest">Card Information</h2>
            {cardSection}
          </section>
          <section className="mb-6">
            <h2 className="mb-3 text-xs font-bold text-gray-400 uppercase tracking-widest">Delivery Information</h2>
            {deliverySection}
          </section>
        </div>
      ...
      <div className="hidden md:flex min-h-screen bg-gray-50 items-center justify-center p-8">
        <div className="w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex">
          ...
          <div className="w-[42%] flex-shrink-0 flex flex-col items-center justify-between p-10"
            ...
            {/* bottom: title */}
            <div className="w-full">
              <p className="text-white/40 text-xs uppercase tracking-widest">Step 1 of 3</p>
              <h2 className="text-white text-xl font-bold mt-1">Payment Details</h2>
            </div>
          </div>

          {/* ── Right: form ── */}
          <div id="desktop-form" className="flex-1 flex flex-col overflow-hidden">
            <div className="flex-1 overflow-y-auto px-10 py-8">
              <section className="mb-7">
                <h2 className="mb-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Personal Info</h2>
                {personalSection}
              </section>
              <section className="mb-7">
                <h2 className="mb-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Card Information</h2>
                {cardSection}
              </section>
              <section className="mb-7">
                <h2 className="mb-4 text-xs font-bold text-gray-400 uppercase tracking-widest">Delivery Information</h2>
                {deliverySection}
              </section>
            </div>
