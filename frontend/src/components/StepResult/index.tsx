...
function ReceiptRow({ label, value, muted = false, mono = false }: {
  label: string; value: string; muted?: boolean; mono?: boolean;
}) {
  return (
    ...
    <div className="rounded-2xl border border-gray-100 bg-gray-50 p-5">
      ...
      <div className="flex flex-col gap-3">
        ...
        {transaction.cardLastFour && (
          <ReceiptRow label="Card"
            value={`${transaction.cardBrand ?? ''} •••• ${transaction.cardLastFour}`.trim()} />
        )}
        <ReceiptRow label="Product" value={fmt(transaction.productAmountInCents)} />
        <ReceiptRow label="Base fee" value={fmt(transaction.baseFeeInCents)} muted />
        <ReceiptRow label="Delivery fee" value={fmt(transaction.deliveryFeeInCents)} muted />
        <div className="mt-1 border-t border-gray-200 pt-3 flex items-center justify-between">
          <span className="text-sm font-bold text-gray-900">Total</span>
          <span className="text-base font-bold text-gray-900">{fmt(transaction.amountInCents)}</span>
        </div>
      </div>
    ...
    <>
      ...
      <div className="hidden md:flex min-h-screen bg-gray-50 items-center justify-center p-8">
        <div className="w-full max-w-4xl bg-white rounded-3xl shadow-xl overflow-hidden flex">
          ...
          {/* ── Left: status panel ── */}
          <div className={`w-[45%] flex-shrink-0 flex flex-col items-center justify-center text-center p-12 gap-6 border-r border-gray-100 relative overflow-hidden`}>
            {/* top accent bar */}
            <div className={`absolute top-0 left-0 right-0 h-1 ${cfg.accentClass}`} />
            {cfg.icon}
            <div className="flex flex-col gap-3">
              <h1 className="text-2xl font-bold text-gray-900">{cfg.headline}</h1>
              <p className="text-sm text-gray-500 leading-relaxed">{cfg.sub}</p>
            </div>
            <span className={`rounded-full px-5 py-2 text-xs font-bold uppercase tracking-widest ${cfg.badgeClass}`}>
              {status}
            </span>
          </div>
          ...
          <div className="flex-1 flex flex-col justify-between p-10">
            <div>
              <h2 className="text-base font-bold text-gray-900 mb-5">Receipt</h2>
              {receipt}
            </div>
