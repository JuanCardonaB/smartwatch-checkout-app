...
function LineItem({ label, amount, muted = false }: { label: string; amount: number; muted?: boolean }) {
  return (
    <div className="flex items-center justify-between">
      <span className={`text-sm ${muted ? 'text-gray-400' : 'text-gray-700'}`}>{label}</span>
      <span className={`text-sm font-medium ${muted ? 'text-gray-400' : 'text-gray-700'}`}>{fmt(amount)}</span>
    </div>
  );
}
...
export default function StepSummary() {
  ...
  function handleBack() { dispatch(setStep(2)); }
  ...
  const cardBlock = (
    <div className="rounded-xl bg-gray-50 border border-gray-100 px-4 py-3">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-gray-900">•••• •••• •••• {lastFour}</p>
  ...
  return (
    <>
      ...
      <div className="md:hidden fixed inset-0 flex flex-col overflow-hidden">
        ...
        {/* front layer */}
        <div className="flex flex-1 flex-col rounded-t-3xl bg-white shadow-2xl overflow-hidden" style={{ marginTop: '-20px' }}>
          ...
          <div className="flex-shrink-0 border-t border-gray-100 bg-white px-5 pb-6 pt-4">
            {payButton}
          </div>
        </div>
      </div>
      ...
      <div className="hidden md:flex min-h-screen bg-gray-50 items-center justify-center p-8">
        <div className="w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex">
          ...
          {/* ── Left: product image ── */}
          <div className="relative w-[55%] flex-shrink-0 bg-gray-100 min-h-[580px]">
            <img src={heroImage} alt={product.name}
              className="absolute inset-0 h-full w-full object-cover" draggable={false} />
            ...
            <div className="absolute bottom-8 left-8 right-8">
              <p className="text-xs text-white/50 uppercase tracking-widest font-medium mb-1">Ordering</p>
