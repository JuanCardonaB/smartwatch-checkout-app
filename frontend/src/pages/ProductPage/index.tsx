...
export default function ProductPage() {
  ...
  function onSheetDragMove(clientY: number) {
    ...
    setSheetHeight(Math.min(MAX_HEIGHT, Math.max(MIN_HEIGHT, dragStartHeight.current + delta)));
  }
  ...
  /* ── swipe ── */
  function onSwipeStart(clientX: number) { swipeStartX.current = clientX; }
    ...
    <>
      ...
      <div className="md:hidden relative h-screen w-full overflow-hidden">
        ...
        <div
          ...
          onTouchEnd={(e) => onSwipeEnd(e.changedTouches[0].clientX, images.length)}
        ...
        >
          ...
          <div className="absolute left-1/2 -translate-x-1/2 flex gap-2 z-10" style={{ bottom: "42%" }}>
            {images.map((_, i) => (
              <button
                key={i}
      ...
      <div className="hidden md:flex min-h-screen bg-gray-50 items-center justify-center p-8">
        <div className="w-full max-w-5xl bg-white rounded-3xl shadow-xl overflow-hidden flex">
          ...
          {/* ── Left: image carousel ── */}
          <div
            className="relative w-[55%] flex-shrink-0 bg-gray-100 select-none overflow-hidden cursor-grab active:cursor-grabbing"
            onMouseDown={(e) => onSwipeStart(e.clientX)}
            onMouseUp={(e) => onSwipeEnd(e.clientX, images.length)}
          >
            {images.map((src, i) => (
          ...
          {/* ── Right: product info ── */}
          <div className="flex-1 flex flex-col justify-between p-10 min-h-[540px]">
            <div>
